...
import (
	...
	mcrepo "github.com/DEEIX-AI/DEEIX-Chat/backend/internal/infra/persistence/postgres/managedcontext"
	...
	wechatbotrepo "github.com/DEEIX-AI/DEEIX-Chat/backend/internal/infra/persistence/postgres/wechatbot"
	...
	managedcontexthttp "github.com/DEEIX-AI/DEEIX-Chat/backend/internal/transport/http/managedcontext"
	...
	wechatbothhttp "github.com/DEEIX-AI/DEEIX-Chat/backend/internal/transport/http/wechatbot"
...
func NewApp() (*App, error) {
	...
	wechatBotRepo := wechatbotrepo.NewRepo(db)
	wechatBotService := appwechatbot.NewService(runtimeCfg, wechatBotRepo, log)
	wechatBotService.SetConversationService(conversationService)
	wechatBotService.SetConversationRepo(conversationRepo)
	wechatBotService.SetChannelService(channelService)
	wechatBotService.SetAuditWriter(auditService)
	wechatBotService.SetAuditReader(auditService)
	wechatBotHandler := wechatbothhttp.NewHandler(wechatBotService)
	wechatBotModule := wechatbothhttp.NewModule(wechatBotHandler)
	authHandler.SetWeChatBotAccessChecker(channelService.CanUseWeChatBot)

	mcRepo := mcrepo.NewRepo(db)
	mcService := appmc.NewService(mcRepo, channelRepo, auditService, cfg.DataEncryptionKey, log)
	mcHandler := managedcontexthttp.NewHandler(mcService)
	mcModule := managedcontexthttp.NewModule(mcHandler)

	conversationService.SetManagedContextRepo(mcRepo)
	...
	adminHandler.SetLLMPoolProber(appllmpool.NewService(channelRepo, log))
	...
	go mcService.RunMetricsCollector(backgroundCtx)
	conversationService.StartBackgroundWorkers(backgroundCtx)
	if cfg.WeChatBotEnabled {
		go func() {
			if err := wechatBotService.StartBot(backgroundCtx); err != nil {
				log.Error("wechat_bot_start_failed", zap.Error(err))
			}
		}()
	}
