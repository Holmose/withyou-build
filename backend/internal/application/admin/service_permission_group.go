...
type permissionGroupRepo interface {
	...
	UpdatePermissionGroup(ctx context.Context, id uint, name string, description string, rateMultiplierPercent int, wechatBotEnabled bool) (*domainchannel.PermissionGroup, error)
...
func (s *Service) CreatePermissionGroup(ctx context.Context, name, description string, rateMultiplierPercent int, wechatBotEnabled bool) (*domainchannel.PermissionGroup, error) {
	if s.permissionGroupRepo == nil {
		return nil, ErrPermissionGroupRepoUnavailable
	}
	...
	item := &domainchannel.PermissionGroup{
		...
		WeChatBotEnabled:      wechatBotEnabled,
	}
...
func (s *Service) UpdatePermissionGroup(ctx context.Context, id uint, name, description string, rateMultiplierPercent int, wechatBotEnabled bool) (*domainchannel.PermissionGroup, error) {
	if s.permissionGroupRepo == nil {
		return nil, ErrPermissionGroupRepoUnavailable
	}
	trimmedName := strings.TrimSpace(name)
	...
	item, err := s.permissionGroupRepo.UpdatePermissionGroup(ctx, id, trimmedName, strings.TrimSpace(description), normalizedPercent, wechatBotEnabled)
	if err != nil {
