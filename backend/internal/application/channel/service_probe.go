...
func (s *Service) TestModel(ctx context.Context, modelID uint, input ModelProbeInput) (*ModelProbeResult, error) {
	...
	for _, row := range rows {
		...
		return s.probeRoute(ctx, row, input.UserID)
	}
	if firstFailure != nil {
...
func (s *Service) TestModelAll(ctx context.Context, modelID uint, input ModelProbeInput) (*ModelProbeBatchResult, error) {
	...
	for index, row := range routes {
		...
		go func(index int, row repository.ChannelUpstreamRouteRow) {
			...
			result, err := s.probeRoute(ctx, row, input.UserID)
			if err != nil {
				errMu.Lock()
				if firstErr == nil {
					firstErr = err
...
func (s *Service) probeRoute(ctx context.Context, row repository.ChannelUpstreamRouteRow, userID uint) (*ModelProbeResult, error) {
	...
	jwtSecret := s.cfg.Snapshot().JWTSecret
	routeConfig := modelProbeRouteConfig(resolved, attributionReferer, attributionTitle, jwtSecret)
	input := llm.GenerateInput{
		UserID:         userID,
		ConversationID: userID, // ponytail: probe has no real conversation, reuse UserID
...
func modelProbeRouteConfig(route *ResolvedRoute, attributionReferer string, attributionTitle string, jwtSecret string) llm.RouteConfig {
