...
type permissionGroupRepo interface {
	...
	ListWeChatBotEnabledGroupIDs(ctx context.Context) ([]uint, error)
}
...
// CanUseWeChatBot 判断用户是否有权限使用微信机器人。
// 用户有效权限组由手动成员、默认权限组和订阅绑定权限组组成，任一启用微信机器人的组即放行。
// 订阅过期、被移出组或关闭开关后实时失效。
func (s *Service) CanUseWeChatBot(ctx context.Context, userID uint) (bool, error) {
	if s.permGroupRepo == nil {
		return false, nil
	}
	enabledGroupIDs, err := s.permGroupRepo.ListWeChatBotEnabledGroupIDs(ctx)
	if err != nil {
		return false, err
	}
