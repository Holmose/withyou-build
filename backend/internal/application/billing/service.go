...
// UsageLeaderboardFilter 描述管理员用量排行榜筛选条件。
type UsageLeaderboardFilter struct {
	StartAt time.Time
	EndAt   time.Time
	Query   string
}

// UsageTrendFilter 描述用量趋势聚合筛选条件。
type UsageTrendFilter struct {
	StartAt time.Time
	EndAt   time.Time
	UserID  uint
...
func (s *Service) currentPeriodPlan(
	...
	// 额度周期跟随订阅：从订阅当前周期开始，到订阅到期结束。
	// 不用日历月边界，避免激活前的用量被计入套餐额度、订阅跨月时额度提前重置。
	startAt := subscription.CurrentPeriodStartAt
...
// ListUsageLeaderboard 分页查询管理员用量排行榜（按累计费用降序）。
func (s *Service) ListUsageLeaderboard(ctx context.Context, page int, pageSize int, filter UsageLeaderboardFilter) ([]domainbilling.UsageLeaderboardEntry, int64, error) {
	offset, limit := normalizePage(page, pageSize)
	return s.repo.ListUsageLeaderboard(ctx, repository.UsageLeaderboardFilter{
...
// ListUsageTrend 查询按日/月聚合的用量趋势。
func (s *Service) ListUsageTrend(ctx context.Context, filter UsageTrendFilter) ([]domainbilling.UsageTrendBucket, error) {
	return s.repo.ListUsageTrend(ctx, repository.UsageTrendFilter{
		StartAt: filter.StartAt,
		EndAt:   filter.EndAt,
		UserID:  filter.UserID,
