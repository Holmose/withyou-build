...
// SetManagedContextRepo injects the managed context repository for upstream context state tracking.
func (s *Service) SetManagedContextRepo(mcRepo repository.ManagedContextRepository) {
	s.mcRepo = mcRepo
}
...
type SendMessageInput struct {
	...
	BranchReason            string
	Cancelable              bool
	// WithYouPoolProfile / WithYouBotID 为微信渠道透传字段（P1 跨仓协商），
	// 非空时以 X-WithYou-Pool-Profile / X-WithYou-Bot-ID 头发送给 WithYou upstream。
