...
// emitProcessUpdateFromPayload 透传上游（如 withyou-runtime）的 process_update 事件
func (r *messageTraceRecorder) emitProcessUpdateFromPayload(payload map[string]interface{}) error {
	if !r.visible() {
		return nil
	}

	// 转换格式：把 block 转换成 trace.tools 供前端使用
	block := payload["block"]
	if block != nil {
		if blockMap, ok := block.(map[string]interface{}); ok {
			...
			payload["trace"] = trace
			r.service.logger.Debug("emitProcessUpdateFromPayload", zap.Any("summary", blockMap["summary"]))
		}
	}

	emitEvent(r.onEvent, "process_update", payload)
	return nil
}
