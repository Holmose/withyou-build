...
// ListConversationsByTimeRange 按主键游标 + 时间范围分页列出会话（管理员按时间导出用）。
func (s *Service) ListConversationsByTimeRange(ctx context.Context, afterID uint, limit int, startTime, endTime *time.Time) ([]model.Conversation, error) {
	return s.repo.ListConversationsByTimeRange(ctx, afterID, limit, startTime, endTime)
}
...
// GetConversationByID 通过 ID 查询会话（绕过用户归属检查，仅限 admin）。
func (s *Service) GetConversationByID(ctx context.Context, conversationID uint) (*model.Conversation, error) {
	item, err := s.repo.GetConversationByID(ctx, conversationID)
	if err != nil {
		return nil, ErrConversationNotFound
	}
	return item, nil
