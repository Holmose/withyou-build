...
func TestHistoryMessagesFromDomainCarriesTimestamp(t *testing.T) {
	created := time.Date(2026, 8, 19, 8, 4, 7, 0, time.UTC)
	messages := []model.Message{
		{Role: "user", Content: "question", CreatedAt: created},
	}

	got := historyMessagesFromDomain(messages, historyMessageOptions{})

	if len(got) != 1 {
		t.Fatalf("expected 1 history message, got %d", len(got))
	}
	if got[0].Timestamp != created.UnixMilli() {
		t.Fatalf("expected timestamp %d, got %d", created.UnixMilli(), got[0].Timestamp)
	}
}
