...
func validatePatchItem(item PatchItem) error {
	...
	case "billing:payment_providers":
		for _, provider := range normalizePaymentProvidersSetting(value) {
			switch provider {
			case "stripe", "epay", "cardkey":
			default:
				return fmt.Errorf("%s must contain only: stripe, epay, cardkey", key)
			}
		}
		return nil
...
func (s *Service) validateBillingPaymentSettings(ctx context.Context, patches []PatchItem) error {
	...
	for _, provider := range providers {
		...
		case "cardkey":
			if err := requireSettingFields(next, []requiredSettingField{
				{key: "billing:cardkey_url", label: "Cardkey URL"},
			}); err != nil {
				return err
			}
		default:
			return fmt.Errorf("billing:payment_providers must contain only: stripe, epay, cardkey")
		}
	}
	return nil
}
