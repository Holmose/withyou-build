...
func (s *Service) sendEmailVerificationCode(to string, code string, template verificationEmailTemplate, logLabel string) error {
	...
	brandName := verificationEmailBrandName(cfg.AppName)
	template.Subject = brandName + " " + strings.TrimSpace(template.Subject)
	message := buildVerificationEmailMessage(parsedFrom.String(), normalizedTo, code, brandName, template, publicAssetURL(cfg.PublicWebBaseURL, "logo.svg"))
...
func buildVerificationEmailMessage(from string, to string, code string, brandName string, template verificationEmailTemplate, logoURL string) string {
	plainBody := buildVerificationPlainText(code, brandName, template)
	htmlBody := buildVerificationHTML(code, brandName, template, logoURL)
...
func buildVerificationPlainText(code string, brandName string, template verificationEmailTemplate) string {
...
10 分钟内有效，请不要泄露给任何人。
	...
	%s`, verificationEmailBrandName(brandName), template.Title, strings.TrimSpace(code), template.SecurityNote)
}

func buildVerificationHTML(code string, brandName string, template verificationEmailTemplate, logoURL string) string {
	...
	logoHTML := verificationEmailLogoHTML(brandName, logoURL)
...
func verificationEmailLogoHTML(brandName string, logoURL string) string {
	escapedBrandName := html.EscapeString(verificationEmailBrandName(brandName))
	if trimmed := strings.TrimSpace(logoURL); trimmed != "" {
		return fmt.Sprintf(`<img src="%s" width="150" alt="%s" style="display:block;width:150px;height:auto;border:0;outline:none;text-decoration:none;">`, html.EscapeString(trimmed), escapedBrandName)
	}
	return fmt.Sprintf(`<div style="font-size:20px;line-height:1.3;font-weight:700;color:#26231f;">%s</div>`, escapedBrandName)
}

func verificationEmailBrandName(value string) string {
