...
func TestBuildVerificationEmailMessageEncodesChineseSubject(t *testing.T) {
	message := buildVerificationEmailMessage("WithYou <no-reply@example.com>", "user@example.com", "123456", "WithYou", verificationEmailTemplate{
		Subject:      "WithYou 验证码",
		Title:        "完成邮箱注册",
		SecurityNote: "如果不是您本人操作，请忽略这封邮件。",
	}, "https://chat.withyou.net.cn/logo.svg")

	if !strings.Contains(message, "Subject: =?utf-8?") {
		t.Fatalf("expected encoded utf-8 subject, got:\n%s", message)
	}
	if strings.Contains(message, "Subject: WithYou 验证码") {
	...
	for _, expected := range []string{
		"WithYou",
		`src="https://chat.withyou.net.cn/logo.svg"`,
		`alt="WithYou"`,
	} {
	...
	if strings.Contains(strings.ToLower(message), "deeix") {
		t.Fatalf("expected WithYou-only visible branding, got:\n%s", message)
	}
}

func TestVerificationEmailLogoFallbackUsesConfiguredBrand(t *testing.T) {
	got := verificationEmailLogoHTML("WithYou", "")
	if got != `<div style="font-size:20px;line-height:1.3;font-weight:700;color:#26231f;">WithYou</div>` {
