...
func TestBuildOTPAuthURLPreservesIssuerAccountSeparator(t *testing.T) {
	got := buildOTPAuthURL("WithYou", "user@example.com", "ABCDEF")
	if strings.Contains(got, "WithYou%3A") {
		t.Fatalf("issuer/account separator should stay as ':' in otpauth label: %s", got)
	}
	if !strings.HasPrefix(got, "otpauth://totp/WithYou:") {
	...
	query := parsed.Query()
	if query.Get("secret") != "ABCDEF" || query.Get("issuer") != "WithYou" || query.Get("digits") != "6" || query.Get("period") != "30" {
