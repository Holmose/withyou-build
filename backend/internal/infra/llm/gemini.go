...
func (c *Client) generateGemini(
	...
	req, err := c.newGeminiRequest(requestCtx, http.MethodPost, requestURL, bytes.NewReader(payload), route, HeaderTemplateValues{UserID: input.UserID, ConversationID: input.ConversationID})
	if err != nil {
		return nil, err
	}
...
func (c *Client) generateGeminiStream(
	...
	req, err := c.newGeminiRequest(firstByteCtx, http.MethodPost, requestURL, bytes.NewReader(payload), route, HeaderTemplateValues{UserID: input.UserID, ConversationID: input.ConversationID})
	if err != nil {
		return nil, err
	}
...
func (c *Client) listModelsGemini(ctx context.Context, route RouteConfig) ([]ModelItem, error) {
	...
	req, err := c.newGeminiRequest(requestCtx, http.MethodGet, requestURL, nil, route, modelListHeaderTemplateValues())
	if err != nil {
		return nil, err
	}
