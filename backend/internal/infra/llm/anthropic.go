...
func (c *Client) newAnthropicRequest(
	...
	if err := setAdditionalHeaders(req, route.HeadersJSON, values); err != nil {
		return nil, err
	}
	return req, nil
}
...
func (c *Client) generateAnthropic(
	...
	req, err := c.newAnthropicRequest(requestCtx, http.MethodPost, requestURL, bytes.NewReader(payload), route, HeaderTemplateValues{UserID: input.UserID, ConversationID: input.ConversationID})
	if err != nil {
		return nil, err
	}
...
func (c *Client) generateAnthropicStream(
	...
	req, err := c.newAnthropicRequest(firstByteCtx, http.MethodPost, requestURL, bytes.NewReader(payload), route, HeaderTemplateValues{UserID: input.UserID, ConversationID: input.ConversationID})
	if err != nil {
		return nil, err
	}
...
func (c *Client) listModelsAnthropic(ctx context.Context, route RouteConfig) ([]ModelItem, error) {
	...
	req, err := c.newAnthropicRequest(requestCtx, http.MethodGet, requestURL, nil, route, modelListHeaderTemplateValues())
	if err != nil {
		return nil, err
	}
