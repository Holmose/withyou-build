...
const (
	openRouterDefaultAttributionReferer = "https://withyou.chat"
	openRouterDefaultAttributionTitle   = "WithYou"
...
func (c *Client) generateOpenAICompatible(ctx context.Context, route RouteConfig, input GenerateInput) (*GenerateOutput, error) {
	...
	headersJSON, err := signJWTAuto(route.HeadersJSON, input.UserID, route.JWTSecret)
	if err != nil {
		return nil, err
	}
	if err := setAdditionalHeaders(req, headersJSON, HeaderTemplateValues{UserID: input.UserID, ConversationID: input.ConversationID}); err != nil {
		return nil, err
	}
	if input.ContextState != "" {
		req.Header.Set("X-WithYou-Context", input.ContextState)
	}
	if input.WithYouPoolProfile != "" {
		req.Header.Set("X-WithYou-Pool-Profile", input.WithYouPoolProfile)
	}
	if input.WithYouBotID != "" {
		req.Header.Set("X-WithYou-Bot-ID", input.WithYouBotID)
	}
	...
	output.Debug = debug
	if ack := resp.Header.Get("X-WithYou-Context"); ack != "" {
		output.ContextState = ack
	}
	return output, nil
}
...
func (c *Client) generateStreamOpenAICompatible(
	...
	// 协商 WithYou 富事件（context_compact/process_update）；其他上游忽略未知 X-* 头
	req.Header.Set("X-WithYou-Extended-Events", "1")
	if apiKey := strings.TrimSpace(route.APIKey); apiKey != "" {
		req.Header.Set("Authorization", "Bearer "+apiKey)
	}
	setOpenRouterAttributionHeaders(req, route)
...
func (c *Client) listModelsOpenAICompatible(ctx context.Context, route RouteConfig) ([]ModelItem, error) {
	...
	setOpenRouterAttributionHeaders(req, route)
	if err := setAdditionalHeaders(req, route.HeadersJSON, modelListHeaderTemplateValues()); err != nil {
		return nil, err
	}
...
func consumeOpenAIGenerateStream(
	...
	dispatch := func() error {
		...
		if eventType := strings.TrimSpace(getString(parsed["type"])); eventType == "process_update" {
			if onEvent != nil {
				return onEvent(GenerateStreamEvent{ProcessUpdate: parsed})
			}
			return nil
		}

		// 捕获 WITHYOU 流式回传的上下文状态（event: withyou_context）
		if currentEvent == "withyou_context" {
			result.ContextState = payloadText
			return nil
		}

		// 捕获 WITHYOU 流式结束时发送的 usage（event: withyou_usage）
		if currentEvent == "withyou_usage" {
			...
			if err := json.Unmarshal([]byte(payloadText), &usageData); err == nil {
				result.Usage.InputTokens = usageData.InputTokens
				result.Usage.OutputTokens = usageData.OutputTokens
				result.Usage.CacheReadTokens = usageData.CacheReadTokens
				result.Usage.CacheWriteTokens = usageData.CacheWriteTokens
				result.Usage.ReasoningTokens = usageData.ReasoningTokens
				if onEvent != nil {
