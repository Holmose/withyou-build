...
// TestBuildWithYouCapabilityURLSiblingVersion verifies the WITHYOU capability
// endpoint uses the sibling versioned path so /v1 bases do NOT become /v1/v1/...
// See: P0-3 in managed-context gap analysis.
func TestBuildWithYouCapabilityURLSiblingVersion(t *testing.T) {
	cases := []struct {
		name    string
		baseURL string
		want    string
		...
		{
			name:    "plain base gets v1 sibling",
			baseURL: "http://withyou-agent:8080",
			want:    "http://withyou-agent:8080/v1/withyou/capabilities",
		},
		{
			name:    "v1 base does NOT double version",
			baseURL: "http://withyou-agent:8080/v1",
			want:    "http://withyou-agent:8080/v1/withyou/capabilities",
		},
		{
			name:    "trailing slash on v1 base is trimmed",
			baseURL: "http://withyou-agent:8080/v1/",
			want:    "http://withyou-agent:8080/v1/withyou/capabilities",
		},
		{
			name:    "nested v1 base preserved",
			baseURL: "http://withyou-agent:8080/custom/path/v1",
			want:    "http://withyou-agent:8080/custom/path/v1/withyou/capabilities",
		},
	}
	for _, tt := range cases {
		t.Run(tt.name, func(t *testing.T) {
			got := BuildVersionedEndpointURL(tt.baseURL, "v1", "/withyou/capabilities")
			if got != tt.want {
