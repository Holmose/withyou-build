...
type Message struct {
	...
	CacheControl     *CacheControl // 支持块级缓存的 adapter 可读取该提示
	Timestamp        int64         // UnixMilli 消息时间戳；0 表示未知。WITHYOU 托管上下文 delta 去重依赖它
}
...
type GenerateInput struct {
	...
	// ContextState 是 DEEIX 向 WITHYOU Agent 发出的上下文同步状态（v2 协议）。
	// 非空时：LLM client 将以 X-WithYou-Context 请求头发送给 WITHYOU upstream。
	// 格式为 JSON：{session_id, mode, epoch, sent_at}
	ContextState string
	// WithYouPoolProfile 是 WithYou 池路由覆盖提示（P1 跨仓协商）。
	// 非空时以 X-WithYou-Pool-Profile 头发送；WithYou 未实现该头时按普通未知头忽略，无副作用。
	WithYouPoolProfile string
	// WithYouBotID 标识发起请求的微信 bot（审计用途）。
	// 非空时以 X-WithYou-Bot-ID 头发送。
	WithYouBotID string
}
...
type GenerateOutput struct {
	...
	// ContextState 是 WITHYOU upstream 在响应头 X-WithYou-Context 中回传的上下文同步状态（v2 协议）。
	// 格式为 JSON：{session_id, mode, epoch, sent_at}
	ContextState string
...
type GenerateStreamEvent struct {
	...
	ProcessUpdate         map[string]interface{} // 上游 process_update 事件（如 withyou-runtime 的工具调用状态）
}
