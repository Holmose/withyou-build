...
func upstreamUpdates(input repository.UpdateChannelUpstreamInput) map[string]interface{} {
	...
	if input.ContextStrategy != nil {
		updates["context_strategy"] = *input.ContextStrategy
	}
	return updates
}
...
func (r *Repo) UpdateManagedContextFields(ctx context.Context, upstreamID uint, capsJSON string, checkedAt time.Time, errorCode string) error {
	return translateError(r.db.WithContext(ctx).Model(&model.LLMUpstream{}).
		Where("id = ?", upstreamID).
		Updates(map[string]interface{}{
			"context_capabilities_json":      capsJSON,
			"context_capabilities_checked_at": checkedAt,
			"context_capabilities_error":     errorCode,
		}).Error)
}

func (r *Repo) UpdateManagedContextRolloutState(ctx context.Context, upstreamID uint, state string) error {
	return translateError(r.db.WithContext(ctx).Model(&model.LLMUpstream{}).
		Where("id = ?", upstreamID).
		Update("managed_context_rollout_state", state).Error)
}

func (r *Repo) UpdateManagedContextProtocolVersion(ctx context.Context, upstreamID uint, protocolVersion string) error {
	return translateError(r.db.WithContext(ctx).Model(&model.LLMUpstream{}).
		Where("id = ?", upstreamID).
		Update("context_protocol_version", protocolVersion).Error)
}

func (r *Repo) UpdateManagedContextBuild(ctx context.Context, upstreamID uint, commit, time, version string) error {
	return translateError(r.db.WithContext(ctx).Model(&model.LLMUpstream{}).
		Where("id = ?", upstreamID).
		Updates(map[string]interface{}{
			"context_build_commit":  commit,
			"context_build_time":   time,
	...
	if err := r.db.WithContext(ctx).
		...
		Select(
			"r.id AS route_id, um.id AS upstream_model_id, u.id AS upstream_id, u.name AS upstream_name, "+
				...
				"u.context_strategy AS context_strategy",
		).
		Joins("JOIN llm_platform_models pm ON pm.id = r.platform_model_id").
		Joins("JOIN llm_upstream_models um ON um.id = r.upstream_model_id").
		Joins("JOIN llm_upstreams u ON u.id = um.upstream_id").
...
func toUpstreamDomain(item model.LLMUpstream) domainchannel.Upstream {
	return domainchannel.Upstream{
		...
		ContextProtocolVersion:         item.ContextProtocolVersion,
		ContextBuildCommit:            item.ContextBuildCommit,
		ContextBuildTime:              item.ContextBuildTime,
		ContextBuildVersion:           item.ContextBuildVersion,
		ManagedContextRolloutState:    item.ManagedContextRolloutState,
		ContextCapabilitiesJSON:        item.ContextCapabilitiesJSON,
		ContextCapabilitiesCheckedAt:  item.ContextCapabilitiesCheckedAt,
		ContextCapabilitiesError:      item.ContextCapabilitiesError,
	}
}
