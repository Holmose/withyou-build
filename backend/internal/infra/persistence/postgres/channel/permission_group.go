...
func (r *Repo) CreatePermissionGroup(ctx context.Context, item *domainchannel.PermissionGroup) error {
	...
	entity := model.PermissionGroup{
		...
		WeChatBotEnabled:      item.WeChatBotEnabled,
	}
	if entity.Name == "" {
		return repository.ErrInvalidInput
	}
...
func (r *Repo) UpdatePermissionGroup(ctx context.Context, id uint, name string, description string, rateMultiplierPercent int, wechatBotEnabled bool) (*domainchannel.PermissionGroup, error) {
	result := r.db.WithContext(ctx).
		...
		Updates(map[string]interface{}{
			...
			"we_chat_bot_enabled":     wechatBotEnabled,
		})
	if result.Error != nil {
...
// ListWeChatBotEnabledGroupIDs 返回启用了微信机器人访问的权限组 ID。
func (r *Repo) ListWeChatBotEnabledGroupIDs(ctx context.Context) ([]uint, error) {
	ids := make([]uint, 0)
	if err := r.db.WithContext(ctx).
		Model(&model.PermissionGroup{}).
		Where("we_chat_bot_enabled = ?", true).
		Order("id ASC").
		Pluck("id", &ids).Error; err != nil {
		return nil, translateError(err)
	}
