...
func TestUsageLeaderboardAndTrend(t *testing.T) {
	db := openBillingLeaderboardSQLiteTestDB(t)
	repo := NewRepo(db)
	ctx := context.Background()

	users := []model.User{
		{PublicID: "u1", Username: "alice", DisplayName: "Alice", Email: "alice@example.com"},
		{PublicID: "u2", Username: "bob", DisplayName: "Bob", Email: "bob@example.com"},
	}
	if err := db.Create(&users).Error; err != nil {
		t.Fatalf("create users: %v", err)
	}

	day1 := time.Date(2026, 6, 6, 0, 0, 0, 0, time.UTC)
	day2 := time.Date(2026, 6, 7, 0, 0, 0, 0, time.UTC)
	entries := []model.UsageLedger{
		{UserID: users[0].ID, UsageDate: day1, BillingAt: day1, InputTokens: 100, OutputTokens: 50, CallCount: 1, BilledNanousd: 300},
		{UserID: users[0].ID, UsageDate: day2, BillingAt: day2, InputTokens: 200, OutputTokens: 60, CallCount: 2, BilledNanousd: 700},
		{UserID: users[1].ID, UsageDate: day1, BillingAt: day1, InputTokens: 10, OutputTokens: 5, CallCount: 1, BilledNanousd: 50},
		{UserID: users[1].ID, UsageDate: day1, BillingAt: day1, IsFreeModel: true, InputTokens: 5, OutputTokens: 5, CallCount: 1, BilledNanousd: 0},
	}
	if err := db.Create(&entries).Error; err != nil {
		t.Fatalf("create usage ledgers: %v", err)
	}
	...
	rows, total, err := repo.ListUsageLeaderboard(ctx, repository.UsageLeaderboardFilter{StartAt: start, EndAt: end}, 0, 10)
	if err != nil {
		t.Fatalf("ListUsageLeaderboard() error = %v", err)
	}
	if total != 2 {
		t.Fatalf("leaderboard total = %d, want 2", total)
	}
	if len(rows) != 2 || rows[0].BilledNanousd != 1000 || rows[1].BilledNanousd != 50 {
		t.Fatalf("unexpected leaderboard rows: %+v", rows)
	}
	if rows[0].Username != "alice" || rows[0].RecordCount != 2 {
		t.Fatalf("unexpected leaderboard top entry: %+v", rows[0])
	}

	trend, err := repo.ListUsageTrend(ctx, repository.UsageTrendFilter{StartAt: start, EndAt: end, Bucket: "day"})
	if err != nil {
		t.Fatalf("ListUsageTrend() error = %v", err)
	}
	if len(trend) != 2 {
		t.Fatalf("trend buckets = %d, want 2", len(trend))
	}
	if trend[0].BilledNanousd != 350 || trend[1].BilledNanousd != 700 {
		t.Fatalf("unexpected trend buckets: %+v", trend)
	}

	userTrend, err := repo.ListUsageTrend(ctx, repository.UsageTrendFilter{StartAt: start, EndAt: end, Bucket: "day", UserID: users[0].ID})
	if err != nil {
		t.Fatalf("ListUsageTrend(user) error = %v", err)
	}
	if len(userTrend) != 2 || userTrend[1].BilledNanousd != 700 {
		t.Fatalf("unexpected per-user trend: %+v", userTrend)
	}

	monthTrend, err := repo.ListUsageTrend(ctx, repository.UsageTrendFilter{StartAt: start, EndAt: end, Bucket: "month"})
	if err != nil {
		t.Fatalf("ListUsageTrend(month) error = %v", err)
	}
	if len(monthTrend) != 1 || monthTrend[0].BucketAt.Format("2006-01-02") != "2026-06-01" || monthTrend[0].BilledNanousd != 1050 {
		t.Fatalf("unexpected month trend: %+v", monthTrend)
	}
}

func openBillingLeaderboardSQLiteTestDB(t *testing.T) *gorm.DB {
	t.Helper()

	db, err := gorm.Open(sqlite.Open("file:billing_leaderboard?mode=memory&cache=shared"), &gorm.Config{})
	if err != nil {
	...
	if err := db.AutoMigrate(&model.UsageLedger{}, &model.User{}); err != nil {
