...
func (r *Repo) billingAtDayKeyExpression() string {
	if r.sqliteDialect() {
		return "strftime('%Y-%m-%d', billing_at)"
	}
	return "TO_CHAR(DATE(billing_at), 'YYYY-MM-DD')"
}

func (r *Repo) billingAtMonthKeyExpression() string {
	if r.sqliteDialect() {
		return "strftime('%Y-%m-01', billing_at)"
	}
	return "TO_CHAR(date_trunc('month', billing_at), 'YYYY-MM-DD')"
}
...
// ListUsageLeaderboard 按用户聚合统计付费用量，返回累计费用降序排行。
func (r *Repo) ListUsageLeaderboard(ctx context.Context, filter repository.UsageLeaderboardFilter, offset int, limit int) ([]domainbilling.UsageLeaderboardEntry, int64, error) {
	query := r.db.WithContext(ctx).
		Table("billing_usage_ledgers ul").
		Select("ul.user_id AS user_id, u.username AS username, u.display_name AS display_name, u.email AS email, "+
			"COUNT(*) AS record_count, SUM(ul.input_tokens) AS input_tokens, SUM(ul.output_tokens) AS output_tokens, "+
			"SUM(ul.call_count) AS call_count, SUM(ul.billed_nanousd) AS billed_nanousd").
		Joins("LEFT JOIN identity_users u ON u.id = ul.user_id").
		Where("ul.billing_at >= ? AND ul.billing_at < ? AND ul.is_free_model = ?", filter.StartAt, filter.EndAt, false)
	if search := strings.TrimSpace(filter.Query); search != "" {
		like := "%" + strings.ToLower(search) + "%"
		query = query.Where("LOWER(u.username) LIKE ? OR LOWER(u.display_name) LIKE ? OR LOWER(u.email) LIKE ?", like, like, like)
	}

	var total int64
	countQuery := r.db.WithContext(ctx).Table("billing_usage_ledgers ul2").
		Select("COUNT(DISTINCT ul2.user_id)").
		Joins("LEFT JOIN identity_users u2 ON u2.id = ul2.user_id").
		Where("ul2.billing_at >= ? AND ul2.billing_at < ? AND ul2.is_free_model = ?", filter.StartAt, filter.EndAt, false)
	if search := strings.TrimSpace(filter.Query); search != "" {
		like := "%" + strings.ToLower(search) + "%"
		countQuery = countQuery.Where("LOWER(u2.username) LIKE ? OR LOWER(u2.display_name) LIKE ? OR LOWER(u2.email) LIKE ?", like, like, like)
	}
	...
	if err := query.
		Group("ul.user_id, u.username, u.display_name, u.email").
		Order("billed_nanousd DESC, ul.user_id ASC").
		Offset(offset).Limit(limit).
		Scan(&rows).Error; err != nil {
		return nil, 0, translateError(err)
	}
	results := make([]domainbilling.UsageLeaderboardEntry, 0, len(rows))
	for _, row := range rows {
		results = append(results, domainbilling.UsageLeaderboardEntry{
			UserID:        row.UserID,
...
// ListUsageTrend 按日或月分组聚合付费用量，用于趋势图。
func (r *Repo) ListUsageTrend(ctx context.Context, filter repository.UsageTrendFilter) ([]domainbilling.UsageTrendBucket, error) {
	bucketExpr := r.billingAtDayKeyExpression()
	if strings.TrimSpace(filter.Bucket) == "month" {
		bucketExpr = r.billingAtMonthKeyExpression()
	}
	query := r.db.WithContext(ctx).
		Table("billing_usage_ledgers").
		Select(bucketExpr+" AS bucket_key, COUNT(*) AS record_count, "+
			"SUM(input_tokens) AS input_tokens, SUM(output_tokens) AS output_tokens, "+
			"SUM(call_count) AS call_count, SUM(billed_nanousd) AS billed_nanousd").
		Where("billing_at >= ? AND billing_at < ? AND is_free_model = ?", filter.StartAt, filter.EndAt, false)
	if filter.UserID > 0 {
		query = query.Where("user_id = ?", filter.UserID)
	}
	...
	results := make([]domainbilling.UsageTrendBucket, 0, len(rows))
	for _, row := range rows {
		bucketAt, err := time.Parse("2006-01-02", row.BucketKey)
		if err != nil {
			continue
		}
		results = append(results, domainbilling.UsageTrendBucket{
			BucketAt:      bucketAt,
			RecordCount:   row.RecordCount,
			InputTokens:   row.InputTokens,
			OutputTokens:  row.OutputTokens,
