...
// GetConversationByID 查询会话（绕过用户归属检查，仅限 admin）。
...
func (r *Repo) ListConversationsByTimeRange(ctx context.Context, afterID uint, limit int, startTime, endTime *time.Time) ([]domainconversation.Conversation, error) {
	var rows []models.Conversation
	query := r.db.WithContext(ctx).Order("id ASC").Limit(limit)
	if afterID > 0 {
		query = query.Where("id > ?", afterID)
