...
func migrate(db *gorm.DB, cfg config.Config) error {
	...
	tableComments := map[string]string{
		...
		"wechat_bot_accounts":            "用户微信机器人账号表",
	...
	if err := applyWeChatBotBaseline(db); err != nil {
		return err
	}
...
func applyWeChatBotBaseline(db *gorm.DB) error {
	statements := []string{
		`CREATE INDEX IF NOT EXISTS idx_wechat_bot_accounts_conversation_id
		ON "wechat_bot_accounts" ("conversation_id")`,
		`DROP INDEX IF EXISTS idx_wechat_bot_accounts_bot_open_id`,
		`CREATE UNIQUE INDEX IF NOT EXISTS idx_wechat_bot_accounts_bot_open_id
		ON "wechat_bot_accounts" ("bot_open_id") WHERE "bot_open_id" <> ''`,
		`ALTER TABLE "wechat_bot_accounts" ADD COLUMN IF NOT EXISTS hub_bot_id varchar(64) NOT NULL DEFAULT ''`,
		`ALTER TABLE "wechat_bot_accounts" ADD COLUMN IF NOT EXISTS hub_channel_key varchar(128) NOT NULL DEFAULT ''`,
	}
	for _, statement := range statements {
		if err := db.Exec(statement).Error; err != nil {
			return err
