...
type LLMUpstream struct {
	...
	ContextStrategy                 string `gorm:"size:32;not null;default:'full_history';comment:上下文策略: full_history managed_bootstrap managed_delta"`
	ContextProtocolVersion          string `gorm:"size:64;not null;default:'';comment:托管上下文协议版本"`
	ContextBuildCommit             string `gorm:"size:64;not null;default:'';comment:托管上下文构建commit"`
	ContextBuildTime              string `gorm:"size:64;not null;default:'';comment:托管上下文构建时间"`
	ContextBuildVersion           string `gorm:"size:64;not null;default:'';comment:托管上下文构建版本号"`
	ManagedContextRolloutState     string `gorm:"size:32;not null;default:'disabled';comment:托管上下文发布状态: disabled checking capability_ready enabled migration_paused degraded capability_failed"`
	ContextCapabilitiesJSON         string `gorm:"type:text;not null;default:'{}';comment:最近一次能力检测结果JSON"`
	ContextCapabilitiesCheckedAt   *time.Time `gorm:"comment:最近能力检测时间"`
	ContextCapabilitiesError       string `gorm:"size:256;not null;default:'';comment:最近能力检测错误码"`
}
