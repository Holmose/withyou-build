...
// UsageLeaderboardEntryResponse 管理员排行榜单条记录。
type UsageLeaderboardEntryResponse struct {
	UserID        uint    `json:"userID"`
...
// UsageTrendBucketResponse 用量趋势点。
...
type BillingConfigResponse struct {
	...
	CardkeyURL               string                      `json:"cardkeyURL"`
}
...
// UsageLeaderboardListResponseDoc 用户用量排行榜分页响应。
type UsageLeaderboardListResponseDoc struct {
	ErrorMsg string `json:"errorMsg"`
	Data     struct {
		Total   int64                           `json:"total"`
		Results []UsageLeaderboardEntryResponse `json:"results"`
	} `json:"data"`
}

// UsageTrendListResponseDoc 用量趋势响应。
type UsageTrendListResponseDoc struct {
	ErrorMsg string                     `json:"errorMsg"`
	Data     []UsageTrendBucketResponse `json:"data"`
}
...
func toUsageLeaderboardResponses(items []domainbilling.UsageLeaderboardEntry) []UsageLeaderboardEntryResponse {
	results := make([]UsageLeaderboardEntryResponse, 0, len(items))
	for _, item := range items {
		results = append(results, UsageLeaderboardEntryResponse{
			UserID:        item.UserID,
...
func toUsageTrendResponses(items []domainbilling.UsageTrendBucket) []UsageTrendBucketResponse {
	results := make([]UsageTrendBucketResponse, 0, len(items))
	for _, item := range items {
		results = append(results, UsageTrendBucketResponse{
