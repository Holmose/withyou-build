...
// ListUsageLogs godoc
// @Summary 管理员查询用量明细
// @Description 按用户、模型、时间范围分页查询所有用户用量账本
// @Tags admin-billing
// @Accept json
// @Produce json
// @Security BearerAuth
...
// @Param platform_model query string false "平台模型名"
// @Param billing_mode query string false "计费方式：free/token/call/duration/tiered"
// @Param created_from query string false "起始时间(ISO8601)"
// @Param created_to query string false "结束时间(ISO8601)"
// @Param sort query string false "排序：created_asc/tokens_desc/cost_desc/latency_desc"
// @Param page query int false "页码"
// @Param page_size query int false "每页数量"
// @Success 200 {object} UsageLedgerListResponseDoc
// @Failure 500 {object} ErrorDoc
// @Router /admin/billing/usage-logs [get]
func (h *Handler) ListUsageLogs(c *gin.Context) {
	...
	items, total, err := h.service.ListUsageLogs(c.Request.Context(), page, pageSize, appbilling.UsageLogListFilter{
		Query:             c.Query("query"),
		PlatformModelName: c.Query("platform_model"),
	...
	if err != nil {
		response.Error(c, http.StatusInternalServerError, "list usage logs failed")
		return
	}
	usages := make([]UsageLedgerResponse, 0, len(items))
	for _, u := range items {
...
// ListUsageLeaderboard godoc
// @Summary 管理员查询用户用量排行榜
// @Description 按累计费用降序返回用户用量排名
// @Tags admin-billing
// @Accept json
// @Produce json
// @Security BearerAuth
...
// @Param q query string false "按用户名/邮箱搜索"
// @Param page query int false "页码"
// @Param page_size query int false "每页数量"
// @Success 200 {object} UsageLeaderboardListResponseDoc
// @Failure 500 {object} ErrorDoc
// @Router /admin/billing/usage-leaderboard [get]
func (h *Handler) ListUsageLeaderboard(c *gin.Context) {
	start, end, ok := dateRangeParams(c)
	if !ok {
		response.Error(c, http.StatusBadRequest, "invalid date range")
		return
	}
	page, pageSize := pageParams(c)
	items, total, err := h.service.ListUsageLeaderboard(c.Request.Context(), page, pageSize, appbilling.UsageLeaderboardFilter{
		StartAt: start,
		EndAt:   end,
		Query:   c.Query("q"),
	})
	if err != nil {
		response.Error(c, http.StatusInternalServerError, "list usage leaderboard failed")
		return
	}
	response.SuccessPage(c, total, toUsageLeaderboardResponses(items))
}

// ListUsageTrend godoc
// @Summary 管理员查询用量趋势
// @Description 按日或月聚合返回用量趋势
// @Tags admin-billing
// @Accept json
// @Produce json
// @Security BearerAuth
// @Param bucket query string false "聚合粒度：day/month，默认 day"
// @Param start_date query string true "起始日期(YYYY-MM-DD)"
// @Param end_date query string true "结束日期(YYYY-MM-DD)"
// @Param user_id query int false "用户ID（不传=全体）"
// @Success 200 {object} UsageTrendListResponseDoc
// @Failure 500 {object} ErrorDoc
// @Router /admin/billing/usage-leaderboard/trend [get]
func (h *Handler) ListUsageTrend(c *gin.Context) {
	start, end, ok := dateRangeParams(c)
	if !ok {
		response.Error(c, http.StatusBadRequest, "invalid date range")
		return
	...
	items, err := h.service.ListUsageTrend(c.Request.Context(), appbilling.UsageTrendFilter{
		StartAt: start,
		EndAt:   end,
		UserID:  userID,
		Bucket:  bucket,
	...
	if err != nil {
		response.Error(c, http.StatusInternalServerError, "list usage trend failed")
		return
	}
	response.Success(c, toUsageTrendResponses(items))
}

func dateRangeParams(c *gin.Context) (time.Time, time.Time, bool) {
