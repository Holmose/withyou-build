...
func resolvePaymentProvider(requested string, enabledProviders []string) (string, error) {
	// cardkey 是纯客户端跳转渠道，没有服务端收银台流程，不允许进入 checkout 下单。
	providers := make([]string, 0, len(enabledProviders))
	for _, provider := range enabledProviders {
		if provider == domainbilling.PaymentProviderCardkey {
			continue
		}
...
func normalizePaymentProviders(raw string) []string {
	...
	for _, part := range parts {
		...
		switch provider {
		case domainbilling.PaymentProviderStripe, domainbilling.PaymentProviderEPay, domainbilling.PaymentProviderCardkey:
		default:
			continue
		}
