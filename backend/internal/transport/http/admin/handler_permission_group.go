...
func (h *Handler) CreatePermissionGroup(c *gin.Context) {
	...
	item, err := h.service.CreatePermissionGroup(c.Request.Context(), req.Name, req.Description, req.RateMultiplierPercent, req.WeChatBotEnabled)
	if err != nil {
		h.writePermissionGroupError(c, err)
		return
	}
...
func (h *Handler) UpdatePermissionGroup(c *gin.Context) {
	...
	item, err := h.service.UpdatePermissionGroup(c.Request.Context(), groupID, req.Name, req.Description, req.RateMultiplierPercent, req.WeChatBotEnabled)
	if err != nil {
		h.writePermissionGroupError(c, err)
		return
	}
