...
type conversationExporter interface {
	...
	ListConversationsByTimeRange(ctx context.Context, afterID uint, limit int, startTime, endTime *time.Time) ([]domainconversation.Conversation, error)
}
...
// ExportConversations 流式导出全量对话，支持按时间范围过滤。
func (h *Handler) ExportConversations(c *gin.Context) {
	...
	if raw := c.Query("start_time"); raw != "" {
		...
		if err != nil {
			response.Error(c, http.StatusBadRequest, "invalid start_time format, use RFC3339 (e.g. 2024-01-01T00:00:00Z)")
			return
		}
		startTime = &t
	}
	if raw := c.Query("end_time"); raw != "" {
		t, err := time.Parse(time.RFC3339, raw)
		if err != nil {
			response.Error(c, http.StatusBadRequest, "invalid end_time format, use RFC3339 (e.g. 2024-01-01T00:00:00Z)")
			return
		}
		endTime = &t
	}
	stripBase64 := c.Query("strip_base64") == "1"
	...
	listConversations := func(ctx context.Context, afterID uint) ([]domainconversation.Conversation, error) {
		if startTime != nil || endTime != nil {
			return h.conversationExport.ListConversationsByTimeRange(ctx, afterID, batchSize, startTime, endTime)
		}
		return h.conversationExport.ListAllConversationsAfterID(ctx, afterID, batchSize)
	}
	...
	for {
		...
		conversations, err := listConversations(c.Request.Context(), lastID)
		if err != nil {
		...
		for i := range conversations {
			...
			if stripBase64 {
				for j := range result.Messages {
					result.Messages[j].Content = stripBase64Images(result.Messages[j].Content)
				}
			}
...
var base64ImageRE = regexp.MustCompile(`data:image/[^;]+;base64,[A-Za-z0-9+/=\s]+`)

func stripBase64Images(content string) string {
	return base64ImageRE.ReplaceAllString(content, "[image]")
}
