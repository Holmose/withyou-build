...
type Handler struct {
	...
	weChatBotAccessChecker func(ctx context.Context, userID uint) (bool, error)
}
...
// SetWeChatBotAccessChecker 注入微信机器人访问权限检查器。
func (h *Handler) SetWeChatBotAccessChecker(checker func(ctx context.Context, userID uint) (bool, error)) {
	h.weChatBotAccessChecker = checker
}
...
func (h *Handler) Me(c *gin.Context) {
	...
	resp := MeResponse{User: toUserResponse(view)}
	if h.weChatBotAccessChecker != nil {
		enabled, checkErr := h.weChatBotAccessChecker(c.Request.Context(), userID)
		if checkErr != nil {
			response.Error(c, http.StatusInternalServerError, "failed to resolve wechat access")
			return
		}
		resp.User.WeChatBotEnabled = enabled
	}
